...
class CreateChildRecordAPIView(generics.CreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        ActivityFeeds.objects.create(
            ...
            activity=f"{self.request.user} added a new Child Record: '{child_record.child_name} — {child_record.dob}'",
        )
        logger.debug(
            f"Activity Feed({self.request.user} added a new Child Record: '{child_record.child_name} — {child_record.dob}') created."
        )
...
class EditChildRecordAPIView(generics.UpdateAPIView):
    ...
    def perform_update(self, serializer):
        ...
        if changes:
            ActivityFeeds.objects.create(
                creator=self.request.user,
                activity=f"{self.request.user} updated Child Record '{previous_child_record.child_name} — {previous_child_record.dob}': {changes}",
            )
            logger.debug(
                f"Activity Feed({self.request.user} updated Child Record '{previous_child_record.child_name} — {previous_child_record.dob}': {changes}) created."
            )
...
class DeleteChildRecordAPIView(generics.DestroyAPIView):
    ...
    def perform_destroy(self, instance):
        ...
        ActivityFeeds.objects.create(
            creator=self.request.user,
            activity=f"The Child Record '{instance.child_name} — {instance.dob}' was deleted by {self.request.user}",
        )
        logger.debug(
            f"Activity feed(The Absences '{instance.child_name} — {instance.dob}' was deleted by {self.request.user}) created."
        )
