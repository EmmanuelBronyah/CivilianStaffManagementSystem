...
class CreateIncompleteTerminationOfAppointmentRecordsAPITest(EmployeeBaseAPITestCase):

    def setUp(self):
        self.create_incomplete_termination_of_appointment_url = reverse(
            "create-incomplete-termination-of-appointment"
        )
        self.create_employee_url = reverse("create-employee")

        CausesOfTermination.objects.create(termination_cause="Retired")
        TerminationStatus.objects.create(termination_status="Resigned")
        FlagType.objects.create(flag_type="Incomplete Record")

        self.termination_record = {
            "employee": "000993",
            "service_id": "000993",
            "cause": 1,
            "status": 1,
    ...
    def test_successful_termination_record_creation(self):
        # Send create employee request
        self.client.post(self.create_employee_url, self.employee_data, format="json")

        # Send create request
        response = self.client.post(
            self.create_incomplete_termination_of_appointment_url,
            self.termination_record,
            format="json",
        )
        ...
        self.assertTrue(
            IncompleteTerminationOfAppointmentRecords.objects.filter(id=1).exists()
        )
        self.assertIn(
            "added a new Incomplete Termination Of Appointment", activity_feed
        )
        self.assertIn(
            "Incomplete termination of appointment records was flagged", last_feed
        )

    def test_create_termination_record_with_invalid_data(self):
        # Send create employee request
        self.client.post(self.create_employee_url, self.employee_data, format="json")

        service_record = self.termination_record.copy()
        service_record["date"] = "2027-09-07"
...
class RetrieveIncompleteTerminationOfAppointmentRecordsAPITest(EmployeeBaseAPITestCase):
    ...
    def setUp(self):
        ...
        self.retrieve_service_with_forces_record_url = reverse(
            "retrieve-incomplete-termination-of-appointment", kwargs={"pk": 1}
        )
        FlagType.objects.create(flag_type="Incomplete Record")
        CausesOfTermination.objects.create(termination_cause="Retired")
        TerminationStatus.objects.create(termination_status="Resigned")
    ...
    def test_get_existing_termination_record(self):
        # Send create employee request
        self.client.post(self.create_employee_url, self.employee_data, format="json")
    ...
    def test_get_non_existing_termination_record(self):
        # Send get request
        response = self.client.get(
            self.retrieve_service_with_forces_record_url, format="json"
        )
...
class EditIncompleteTerminationOfAppointmentRecordsAPITest(EmployeeBaseAPITestCase):
    ...
    def setUp(self):
        ...
        self.edit_service_with_forces_record_url = reverse(
            "edit-incomplete-termination-of-appointment", kwargs={"pk": 1}
        )
        FlagType.objects.create(flag_type="Incomplete Record")
        CausesOfTermination.objects.create(termination_cause="Retired")
        TerminationStatus.objects.create(termination_status="Resigned")
    ...
    def test_edit_termination_record(self):
        # Send create employee request
        self.client.post(self.create_employee_url, self.employee_data, format="json")
        ...
        self.assertIn("updated Incomplete Termination Of Appointment", activity_feed)
    ...
    def test_all_fields_none(self):
        ...
        # Send patch/edit request
        edit_data = self.termination_record.copy()
...
class DeleteIncompleteTerminationOfAppointmentRecordsAPITest(EmployeeBaseAPITestCase):
    ...
    def setUp(self):
        ...
        self.delete_service_with_forces_record_url = reverse(
            "delete-incomplete-termination-of-appointment", kwargs={"pk": 1}
        )
        FlagType.objects.create(flag_type="Incomplete Record")
        CausesOfTermination.objects.create(termination_cause="Retired")
        TerminationStatus.objects.create(termination_status="Resigned")
    ...
    def test_successful_deletion(self):
        ...
        self.assertEqual(
            "The Incomplete Termination Of Appointment(ID: 1) was deleted by Standard User",
            activity_feed,
        )
        self.assertIn(
            "Incomplete termination of appointment records flag was deleted by Standard User.",
            last_activity_feed,
        )
